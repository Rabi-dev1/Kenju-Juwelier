      ...
      {buttons.map((btn) => {
        ...
        return (
          <a
            ...
            className={`relative flex items-center justify-center w-10 h-10 transition-all duration-200 hover:-translate-x-0.5 ${isWA ? 'wa-pulse' : ''}`}
            style={{
              background: isHovered
                ? (isWA ? '#25D366' : 'var(--kj-gold)')
                : isWA
                  ? 'rgba(37,211,102,0.12)'
                  : 'var(--kj-card)',
              color: isHovered ? '#fff' : isWA ? '#25D366' : 'var(--kj-muted)',
